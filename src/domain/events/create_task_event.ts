...
import { ChatInputCommandInteraction, LabelBuilder, ModalBuilder, ModalMessageModalSubmitInteraction, ModalSubmitInteraction, SelectMenuBuilder, SlashCommandBuilder, StringSelectMenuBuilder, TextInputBuilder, TextInputStyle } from "discord.js";
...
export class CreateTaskEvent implements IEvent , IModal{
    ...
    async getModal(...args: any[]): Promise<ModalBuilder> {
        ...
        const modal = new ModalBuilder().setCustomId(this.getModalID()).setTitle("Create New Task");
        ...
        .setStringSelectMenuComponent(new StringSelectMenuBuilder().setCustomId("project-selected").setOptions(
            project
                .map((p) => {
                    return {
                        label: p.name,
        ...
        modal.addLabelComponents(projectSelectedInput , taskNameInput , taskDescriptionInput , taskDeadlineInput);
        return modal;    
    ...
    getSlashCommand() {
        return new SlashCommandBuilder().setName("create-task").setDescription("Create a new task") ;    
    }
