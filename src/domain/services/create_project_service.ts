...
export class CreateProjectService {
  ...
  async createProject(req: CreateProjectRequest): Promise<void> {
    try {
      ...
      const role = await this.createRoleInGuild(
        req.guildId,
        req.projectRoleName,
      );
  ...
  private async createRoleInGuild(
    guildId: string,
    roleName: string,
  ): Promise<Role> {
    ...
    try {
      ...
      throw new DiscordBotError(
        "Role Already Exists",
        `The role <@&${existing.id}> already exists in the guild.`,
      );
    } catch (err: any) {
      if (err instanceof DiscordBotError) {
        console.warn(
          `Handled DiscordBotError in CreateProjectService: ${err.title} - ${err.message}`,
        );
      } else {
